...
import { financeRequests, FeeItem, FeeItemCategory, UpdateFeeItemRequest } from "@/lib/requests/finances"
...
export default function UpdateFeeItemPage() {
    ...
    const fetchFeeItem = async () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setCategory(item.category as FeeItemCategory)
          setDescription(item.description || "")
          setAppliesToAllBranches(item.applies_to_all_branches)
          if (!item.applies_to_all_branches) {
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true)
    try {
      const data: UpdateFeeItemRequest = {
        name: name.trim(),
        default_amount: amount,
        category: category as FeeItemCategory,
        description: description.trim() || undefined,
        branch_ids: appliesToAllBranches ? [] : selectedBranchIds.length > 0 ? selectedBranchIds : undefined,
      }

      await financeRequests.updateFeeItem(feeItemId, data)

      toast.success("Fee item updated successfully!")
      router.push(`/finance/fee-items/${feeItemId}`)
    } catch (err: any) {
      toast.error("Failed to update fee item", { description: err.message || "Something went wrong" })
    } finally {
      setSaving(false)
    ...
    <>
      ...
      <MainContent>
        <div className="container mx-auto px-6 py-8 max-w-4xl">
          <div className="mb-8">
            <Link href={`/finances/fee-items/${feeItemId}`}>
              <Button variant="ghost" size="sm">
                <ArrowLeft className="w-4 h-4 mr-2" />
                Back to Fee Item
              </Button>
            </Link>
            <h1 className="text-3xl font-bold mt-6">Update Fee Item</h1>
            <p className="text-muted-foreground mt-2">
              Edit details for {feeItem.name}
            </p>
          </div>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex justify-end gap-4 pt-8">
                ...
                <Button
                  ...
                  disabled={saving || !name.trim() || !defaultAmount || !category}
                >
                  {saving ? (
                    <>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription className="space-y-4">
                  <p>You are about to update the fee item:</p>
